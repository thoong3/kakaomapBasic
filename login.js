document.addEventListener('DOMContentLoaded', function() {
    ...
    if (loginButton) {
        ...
        // 기존에 저장된 토큰 삭제
            ...
            const redirectUri = 'https://sharemap.kr/callback.html';  // 설정한 리다이렉트 URI
    ...
    if (logoutButton) {
        logoutButton.addEventListener('click', () => {
            // 로그아웃 처리 (로컬 스토리지에서 토큰 및 사용자 정보 삭제)
            localStorage.removeItem('accessToken');
            localStorage.removeItem('userInfo');  // 로컬 스토리지에서 사용자 정보 제거
            ...
            // 저장된 데이터 리스트를 초기화 (로그아웃 후)
            displaySavedDataList();  // 저장된 데이터 리스트 다시 불러오기
    ...
    async function requestKakaoToken(code) {
        ...
        const clientId = 'e8b18d033882132011039848cc8b0f58';  // 제공된 REST API 키
        const redirectUri = 'https://sharemap.kr/callback.html';
    ...
    async function requestKakaoUserInfo(token) {
        ...
        if (response.ok) {
            ...
            if (userInfo.properties && userInfo.properties.profile_image) {
                let profileImageUrl = userInfo.properties.profile_image;
            ...
            // 로그인 후 저장된 데이터 리스트 불러오기
            displaySavedDataList();  // 로그인 후 저장된 데이터 리스트 다시 불러오기
